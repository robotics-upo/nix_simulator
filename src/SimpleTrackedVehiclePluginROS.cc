...
#include "nix_simulator/SimpleTrackedVehiclePluginROS.hh"
...
GZ_REGISTER_MODEL_PLUGIN(SimpleTrackedVehiclePluginROS)

SimpleTrackedVehiclePluginROS::~SimpleTrackedVehiclePluginROS()
{
...
void SimpleTrackedVehiclePluginROS::Load(physics::ModelPtr _model,
                                      sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "SimpleTrackedVehiclePluginROS: _model pointer is NULL");
  GZ_ASSERT(_sdf, "SimpleTrackedVehiclePluginROS: _sdf pointer is NULL");

  if (_model->GetWorld()->Physics()->GetType() != "ode")
  {
    gzerr << "Tracked vehicle simulation works only with ODE." << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }

  TrackedVehiclePluginROS::Load(_model, _sdf);

  if (!_sdf->HasElement("body"))
  {
    gzerr << "SimpleTrackedVehiclePluginROS: <body> tag missing." << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }

  if (!_sdf->HasElement("left_track"))
  {
    gzerr << "SimpleTrackedVehiclePluginROS: <left_track> tag missing."
          << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }
  ...
  {
    gzerr << "SimpleTrackedVehiclePluginROS: <right_track> tag missing."
          << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }
  ...
  if (this->body == nullptr)
  {
    gzerr << "SimpleTrackedVehiclePluginROS: <body> link does not exist."
          << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }
  else
  {
    gzmsg << "SimpleTrackedVehiclePluginROS: Successfully added robot body link "
  ...
  {
    gzerr << "SimpleTrackedVehiclePluginROS: <left_track> link does not exist."
          << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }
  else
  {
    gzmsg << "SimpleTrackedVehiclePluginROS: Successfully added left track link "
  ...
  {
    gzerr << "SimpleTrackedVehiclePluginROS: <right_track> link does not exist."
          << std::endl;
    throw std::runtime_error("SimpleTrackedVehiclePluginROS: Load() failed.");
  }
  else
  {
    gzmsg << "SimpleTrackedVehiclePluginROS: Successfully added right track link "
  ...
  if (_sdf->HasElement("left_flipper"))
  {
    ...
    while (flipper)
    {
      ...
      {
        gzerr << "SimpleTrackedVehiclePluginROS: <left_flipper> link '"
              << flipperName << "' does not exist." << std::endl;
      }
      else
      {
        gtracks[Tracks::LEFT].push_back(flipperLink);
        gzmsg << "SimpleTrackedVehiclePluginROS: Successfully added left flipper "
  ...
  if (_sdf->HasElement("right_flipper"))
  {
    ...
    while (flipper)
    {
      ...
      if (flipperLink == nullptr)
      {
        gzerr << "SimpleTrackedVehiclePluginROS: <right_flipper> link '"
              << flipperName << "' does not exist." << std::endl;
      }
      else
      {
        gtracks[Tracks::RIGHT].push_back(flipperLink);
        gzmsg << "SimpleTrackedVehiclePluginROS: Successfully added right flipper "
                 "link '" << flipperName << "'" << std::endl;
      }
...
void SimpleTrackedVehiclePluginROS::Init()
{
  TrackedVehiclePluginROS::Init();
  ...
  this->beforePhysicsUpdateConnection =
      event::Events::ConnectBeforePhysicsUpdate(
          std::bind(&SimpleTrackedVehiclePluginROS::DriveTracks, this,
                    std::placeholders::_1));
}

void SimpleTrackedVehiclePluginROS::Reset()
{
  TrackedVehiclePluginROS::Reset();
}

void SimpleTrackedVehiclePluginROS::SetTrackVelocityImpl(double _left,
                                                      double _right)
{
...
void SimpleTrackedVehiclePluginROS::UpdateTrackSurface()
{
  auto& gtracks = globalTracks.at(this->body);
...
void SimpleTrackedVehiclePluginROS::SetGeomCategories()
{
...
size_t SimpleTrackedVehiclePluginROS::GetNumTracks(const Tracks side) const
{
  auto& gtracks = globalTracks.at(this->body);
  return gtracks[side].size();
}

void SimpleTrackedVehiclePluginROS::DriveTracks(
...
ignition::math::Vector3d SimpleTrackedVehiclePluginROS::ComputeFrictionDirection(
...
double SimpleTrackedVehiclePluginROS::ComputeSurfaceMotion(const double _beltSpeed,
...
SimpleTrackedVehiclePluginROS::ContactIterator
SimpleTrackedVehiclePluginROS::ContactIterator::operator++()
{
...
SimpleTrackedVehiclePluginROS::ContactIterator::ContactIterator()
...
SimpleTrackedVehiclePluginROS::ContactIterator::ContactIterator(
    const SimpleTrackedVehiclePluginROS::ContactIterator &_rhs)
{
...
SimpleTrackedVehiclePluginROS::ContactIterator
SimpleTrackedVehiclePluginROS::ContactIterator::begin(
    dBodyID _body, dGeomID _geom1, dGeomID _geom2)
{
...
SimpleTrackedVehiclePluginROS::ContactIterator
SimpleTrackedVehiclePluginROS::ContactIterator::end()
{
  return ContactIterator(true);
}

bool SimpleTrackedVehiclePluginROS::ContactIterator::operator==(
    const SimpleTrackedVehiclePluginROS::ContactIterator &_rhs)
{
...
SimpleTrackedVehiclePluginROS::ContactIterator
SimpleTrackedVehiclePluginROS::ContactIterator::operator++(int /*_unused*/)
{
...
SimpleTrackedVehiclePluginROS::ContactIterator::reference
SimpleTrackedVehiclePluginROS::ContactIterator::operator*()
{
  if (!this->initialized)
    ++(*this);
...
SimpleTrackedVehiclePluginROS::ContactIterator::pointer
SimpleTrackedVehiclePluginROS::ContactIterator::operator->()
{
  if (!this->initialized)
    ++(*this);
...
SimpleTrackedVehiclePluginROS::ContactIterator::pointer
SimpleTrackedVehiclePluginROS::ContactIterator::getPointer()
{
  if (!this->initialized)
    ++(*this);
...
bool SimpleTrackedVehiclePluginROS::ContactIterator::operator!=(
    const SimpleTrackedVehiclePluginROS::ContactIterator &_rhs)
{
  return !SimpleTrackedVehiclePluginROS::ContactIterator::operator==(_rhs);
}
